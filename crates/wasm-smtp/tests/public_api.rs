...
use wasm_smtp::{
    AuthMechanism, PolicyError, SendOutcome, SessionState, SmtpClient, SmtpClientOptions, SmtpError,
};
...
const BODY: &str = "From: from@example.com\r\nTo: to@example.com\r\nSubject: hi\r\n\r\nHello.\r\n";
...
fn connect_login_send_quit() {
    ...
    let outcome: SendOutcome = block_on(async {
        ...
        let o = c
            .send_mail("from@example.com", &["to@example.com"], BODY)
            .await
            .unwrap();
        c.quit().await.unwrap();
        o
...
fn connect_exposes_capabilities() {
    let (transport, _, _) = TestTransport::new(&[&standard_exchange()]);
    block_on(async {
        let c = SmtpClient::connect(transport, "client.example.com")
            .await
            .unwrap();
        let caps = c.capabilities();
        assert!(
            !caps.is_empty(),
            "capabilities must be populated after connect"
        );
...
fn audit_sink_receives_events() {
    ...
    block_on(async {
        ...
        c.send_mail("from@example.com", &["to@example.com"], BODY)
            .await
            .unwrap();
        c.quit().await.unwrap();
    });
    ...
    for expected in &[
        ...
        assert!(
            events.iter().any(|e| e.starts_with(expected)),
            "missing: {expected}"
        );
    }
}
...
fn login_with_plain_explicit() {
    ...
    block_on(async {
        let mut c = SmtpClient::connect(transport, "client.example.com")
            .await
            .unwrap();
        c.login_with(AuthMechanism::Plain, "user@example.com", "secret")
            .await
            .unwrap();
        c.quit().await.unwrap();
    });
